...
void DesktopMultiWindowPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "createWindow") {
    ...
    result->Success();
  } else if (method_call.method_name() == "setPreventFocus") {
    auto *arguments = std::get_if<flutter::EncodableMap>(method_call.arguments());
    auto window_id = arguments->at(flutter::EncodableValue("windowId")).LongValue();
    MultiWindowManager::Instance()->SetPreventFocus(window_id, arguments);
    result->Success();
    ...
    result->Success();
  } else if (method_call.method_name() == "setSkipTaskbar") {
    auto *arguments = std::get_if<flutter::EncodableMap>(method_call.arguments());
    auto window_id = arguments->at(flutter::EncodableValue("windowId")).LongValue();
